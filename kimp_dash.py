...
from get_gap import get_gap_bnc_df,get_gap_ftx_df,merge_table
...
app.layout = html.Div([
    dcc.Store(id='check1', storage_type='session',data=True),
    dcc.Store(id='check2', storage_type='session',data=False),
    dbc.Row([html.Div([dcc.Interval(id='interval-component',interval=60*1000*3,n_intervals=0),
...
@app.callback(
    Output('merge_table','children'),Output('check2','data'),
    [Input('interval-component','n_intervals'),Input('check2','data')]
)
def refresh_merge(n,check):

    if n==0:
        pass
    elif n!=0 and check==False:
        raise PreventUpdate
